...
public class UserController {
    ...
    public User getById(int user_id){
        ...
        rs = koneksi.executeSelect("SELECT * FROM userd WHERE id= ?", data);
        if(rs != null){
            try{
                while(rs.next()){
                    Timestamp timestamp = rs.getTimestamp(5);
    ...
    public User getLogin(String username, String password){
        ...
        if(rs != null){
            try{
                while(rs.next()){
                    Timestamp timestamp = rs.getTimestamp(5);
                    pengguna = new User(Integer.parseInt(rs.getString(1)), rs.getString(2), rs.getString(3), rs.getString(4), timestamp.toLocalDateTime());
